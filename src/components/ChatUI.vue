...
import { ATT_IMAGE, ATT_FILE, Messages } from '../modules/messages'
...
export default {
    ...
    mounted() {
        ...
        EventBus.$on('user-left', user => this.$delete(this.contacts, user.userId))
        EventBus.$on('webrtc-request', data => new WebRTCSender(this.published.find(pub => pub.id === data.uid)))
